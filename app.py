...
def extract_value(body, keywords):
...
def meal():
    ...
    action_value = extract_value(body, ['오늘', '내일']) or '오늘'
...
def schedule():
    body = request.get_json()
    schedule_type = extract_value(body, ['이번주', '다음주', '이번달'])

    if schedule_type == '이번주':
        start, end = get_week_date_range(0)
    elif schedule_type == '다음주':
        start, end = get_week_date_range(1)
    elif schedule_type == '이번달':
    ...
    if not schedules:
        text = f"{schedule_type} 학사일정이 없습니다."
    else:
